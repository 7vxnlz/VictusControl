...
`DeviceValidatedInputLength` therefore remains unset and normal SetFanMax control remains **NO-GO**. The repeated four-byte physical response permits only a separately approved, command-line-only developer experiment scope; it does not permit normal writes or UI control.
...
The [second four-byte confirmation result](set-fan-max-second-4byte-confirmation-result.md) repeats a physical response for the four-byte hypothesis and supports limited developer-only experiments, but retains NO-GO for normal control. Any further run still requires separate approval, all runtime gates, and append-only evidence logging; it is not a one-byte test, payload validation, or UI-control authorization.
