...
- The [HP UI/usefulness gap audit](hp-victus-ui-usefulness-gap-audit.md) records a remaining functional gap despite that accepted appearance: HP mode lacks live telemetry and proven capability/status for inherited sections. The next safe implementation is live read-only telemetry/status; normal control and package publish remain NO-GO.
...
Implement live read-only HP telemetry/status for CPU/GPU temperature, fan RPM/status, and battery status using reviewed existing/local/OS sources only, as scoped by the [usefulness gap audit](hp-victus-ui-usefulness-gap-audit.md). Unproven readings stay unavailable; no HP WMI write/probe invocation or control route is permitted. Release evidence work remains source-only and all package blockers stay open.
