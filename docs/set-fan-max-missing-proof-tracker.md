...
| --- | --- | --- | --- |
| Device-specific input length | **Missing** | Independent evidence for this exact model/BIOS selecting exactly one input length (`1` or `4`), with no fallback. | [Method/input validation](set-fan-max-method-input-validation.md), [device validation plan](set-fan-max-device-validation-plan.md), [payload-shape decision plan](set-fan-max-payload-shape-decision-plan.md) |
...
All statuses remain unchanged by the runtime flag design and the [payload-shape decision plan](set-fan-max-payload-shape-decision-plan.md). Proposed flags and approval wording are requirements, not proof and not authorization.
