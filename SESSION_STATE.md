...
- Last completed work: created `docs/set-fan-level-first-write-preflight-design.md`; first-write NO-GO, executable candidate set empty, ABI/units/fan mapping and independently justified restore/recovery required. Prior persisted `128` / `80-80` evidence proves serialization only. No product code or readiness changed; `DeviceValidatedInputLength` stays null.
- Files touched recently: new preflight design, dry-run scaffold, comparison, research abstraction, blocker docs, and this state file.
...
- Next likely task: create a documentation-only SetFanLevel ABI/units/fan-ordering and restore/recovery evidence worksheet using the preflight design. Keep unresolved gates blocked and the dry-run disconnected from transport; no numeric first-write candidate is approved.
