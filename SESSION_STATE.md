...
- Last completed work: created `docs/set-fan-level-abi-units-evidence-audit.md` from six read-only local references. Common `0x20008/0x2E` identity, conflicting 2/3-, 4-, and 128-byte requests, mixed scaling/+128 cleaning conventions, and nearby Victus F.30 zero-handoff hazards leave ABI/units/mapping/recovery unproven. `80-80` has uncertain firmware meaning; executable candidate set remains empty, first-write and normal control NO-GO. No product code or readiness changed; `DeviceValidatedInputLength` stays null.
- Files touched recently: new ABI/units audit, preflight design, dry-run scaffold, comparison, research abstraction, blocker docs, and this state file.
- Known build/test status: 2026-09-05 build passed with 0 errors and 4 recurring NU1900 warnings; tests passed 276/276, none skipped. This documentation task ran no CLI dry-run, fan experiment, or WMI method; no publishing or release-artifact creation.
...
- Next likely task: prepare a documentation-only request for existing exact-device ABI/units/high-bit and recovery evidence; resolve the ghelper-omen 128-/four-byte conflict and nearby Victus zero-handoff hazard. No new write/probe request, numeric candidate, or connection from dry-run to transport is approved.
