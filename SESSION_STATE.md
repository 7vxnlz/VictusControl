...
- Current objective: preserve the implemented OS-only HP status display while CPU/GPU temperature and fan RPM stay unavailable pending source proof. Source-only release-prep GO; preview publish and normal fan control NO-GO.
- Last completed work: added a narrow Windows CPU/power telemetry provider, fail-closed formatter, inherited-shell status wiring, Diagnostic source/freshness text, and pure safety/availability tests.
- Files touched recently: `app/Hardware/Hp/HpReadOnlyTelemetry.cs`, `app/Hardware/Hp/HpReadOnlyTelemetryFormatter.cs`, `app/Hardware/Hp/HpWindowsTelemetrySource.cs`, `app/Settings.cs`, focused HP tests, and the five telemetry/readiness documents.
- Known build/test status: 2026-09-04 `dotnet build VictusX.sln` passed with 0 errors and 4 NU1900 warnings; `dotnet test VictusX.sln` passed 220/220, none skipped. Normal `--hp-victus` launch stayed responsive and skipped all explicitly gated BIOS-method probes; desktop automation could not enumerate its untitled borderless window for screenshots.
- Open questions: trustworthy CPU/GPU temperature and fan RPM sources remain unproven; unavailable values must not trigger HP probing or hardware fallbacks. Independent live visual confirmation of the new labels remains pending because the desktop helper could not target the app window.
- Next likely task: design read-only source validation for CPU/GPU temperature and fan RPM, with no HP BIOS-method probe, EC, driver, experiment, or control path.
